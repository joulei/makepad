...
def main():
    ...
    aliases = unicode.aliases(ucd_dir)
    ...
    digit = set()
    digit.update(range(ord('0'), ord('9') + 1))
    digit = sorted(digit)
    ...
    punct.update(categories["S"])
    punct.difference_update(derived_properties["Alphabetic"])
    punct = sorted(punct)
    ...
    xdigit = set()
    xdigit.update(range(ord('0'), ord('9') + 1))
    xdigit.update(range(ord('A'), ord('F') + 1))
    xdigit.update(range(ord('a'), ord('f') + 1))
    xdigit = sorted(xdigit)

    print("//! Generated by:")
    print("//! generate_posix_char_classess.py <ucd_dir>")
    print()

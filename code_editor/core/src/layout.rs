...
impl Layouter {
    ...
    fn layout_virtual_line(
        &mut self,
        virtual_line: &str,
        handle_event: &mut impl FnMut(Event<'_>),
    ) {
        use crate::str::StrExt;

        self.dispatch_event(EventKind::StartOfVirtualLine, virtual_line, handle_event);
        for grapheme in virtual_line.graphemes() {
            self.layout_grapheme(grapheme, handle_event);
        }
        self.dispatch_event(EventKind::EndOfVirtualLine, virtual_line, handle_event);
    ...
    fn layout_grapheme(&mut self, grapheme: &str, handle_event: &mut impl FnMut(Event<'_>)) {
        ...
        self.dispatch_event(EventKind::StartOfGrapheme, grapheme, handle_event);
        self.byte_index += grapheme.len();
        self.position.column_index += grapheme
            .chars()
            .map(|char| char.column_count())
            .sum::<usize>();
        self.dispatch_event(EventKind::EndOfGrapheme, grapheme, handle_event);
    }

...
// IME text state notification (Java→Rust)
#[no_mangle]
...
pub unsafe fn to_java_configure_keyboard(config: &TextInputConfig) {
    ...
    ndk_utils::call_void_method!(
        ...
        config.is_multiline as jni_sys::jboolean as std::ffi::c_uint,
        config.is_secure as jni_sys::jboolean as std::ffi::c_uint
    );
}

/// Update IME text state when Rust changes text outside of IME flow (e.g., clear button)
